...
- Dernière terminée : **le contexte hérité** (`docs/roadmaps/15_inherited_context.md`) — anti-prop-drilling : `Node.Provide<T>` (un ancêtre publie ↓) + `Node.Inherit<T>`/`TryInherit<T>` (un descendant résout le plus proche fournisseur par type ↑, pull ; dict `Type→object` paresseux sur le nœud, remontée `Parent` O(profondeur), introuvable → exception ou `TryInherit` false). v1 = remontée non réactive (valeurs stables ; la réactivité passe par les `Property` de la valeur fournie), re-fourniture réactive différée. `Inherit` exige d'être dans l'arbre → les consommateurs résolvent à `OnAttached`, pas au constructeur. Client : `Editor : Scene` **fournit** l'`EditorState` à tout son sous-arbre ; `HierarchyRow`/`InspectorView` l'**héritent** (plus d'enfilage manuel). Roadmap 14 (**layout**, `docs/roadmaps/14_layout.md`, blocs 1-5) — vrai moteur de layout, modèle **« contraintes ↓ / tailles ↑ »** de Flutter piloté par signals : **une computation de layout par nœud** — `Incoming` (`Property<Constraints>`, posée par le parent) + `CreateEffect(() => Size.Value = ComputeLayout(Incoming.Value))` ; le parent pose `child.Incoming` et lit `child.Size` (acyclique : ↓ Incoming, ↑ Size) ; **skip-unchanged et relayout boundaries gratuits**. `LinearContainer.ComputeLayout` = flex (sonde les non-flex, partage l'espace libre ∝ facteur, remplit l'axe croisé). Flex en **propriétés inline sur `UINode`** (`FlexFactor`/`FlexTight`, modèle CSS `flex-grow` — sur l'enfant, persistent au reparentage), réglables direct (`node.FlexFactor.Value = 1f`) ou via les **descripteurs-sucre** `Expanded`/`Flexible` (pas des `Node`) au `Add` ; `NodeInspector` les découvre comme `Position`/`Size` (toujours visibles). *(L'ancienne `ParentData`/`Node.CreateParentData`/`FlexParentData` — push, par-parent-immédiat — a été retirée : son seul client, le flex, est passé inline.)* `SceneTree.SurfaceSize` sème la racine ; coque éditeur = un `Row` qui reflue (`--demo layout`/`hierarchy`). Pas de `DesiredSize`/`PreferredSize`/`dry`. **Différés** : `MinSize`/`MaxSize` (bornes propres, client `Button`), `Move`/`Moved` (réordonner sans ré-adoption, client glisser-déposer), découplage pompe/rendu (`docs/roadmaps/11_boucle.md`). Roadmap 13 (éditeur MVP) **terminée**.
