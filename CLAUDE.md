...
- Current phase : **le contexte hérité** (`docs/roadmaps/15_inherited_context.md`) — anti-prop-drilling : `Node.Provide<T>`/`Inherit<T>` (un ancêtre fournit ↓, un descendant hérite le plus proche fournisseur par type ↑, pull) ; client : `EditorState` (supprimer son enfilage manuel dans les constructeurs). **Distinct de la parent-data** (pull/par-type/ancêtre-quelconque vs push/par-parent-immédiat). v1 = remontée d'arbre non réactive (valeurs stables ; la réactivité passe par les `Property` de la valeur), re-fourniture réactive différée. Dernière terminée : **le layout** (`docs/roadmaps/14_layout.md`, blocs 1-5) — vrai moteur de layout, modèle **« contraintes ↓ / tailles ↑ »** de Flutter piloté par signals : **une computation de layout par nœud** — `Incoming` (`Property<Constraints>`, posée par le parent) + `CreateEffect(() => Size.Value = ComputeLayout(Incoming.Value))` ; le parent pose `child.Incoming` et lit `child.Size` (acyclique : ↓ Incoming, ↑ Size) ; **skip-unchanged et relayout boundaries gratuits**. `LinearContainer.ComputeLayout` = flex (sonde les non-flex, partage l'espace libre ∝ `Factor`, remplit l'axe croisé). Flex en **enveloppes-descripteurs** `Expanded`/`Flexible` (pas des `Node`) dissoutes en **parent-data** via `virtual Node.CreateParentData()` au goulot d'adoption ; `FlexParentData` = sac de `Property` (`Factor`/`Tight`) ; `NodeInspector` lit la parent-data en plus (contextuel) ; `SceneTree.SurfaceSize` sème la racine ; coque éditeur = un `Row` qui reflue (`--demo layout`/`hierarchy`). Pas de `DesiredSize`/`PreferredSize`/`dry`. **Différés** : `MinSize`/`MaxSize` (bornes propres, client `Button`), `Move`/`Moved` (réordonner sans ré-adoption, client glisser-déposer), découplage pompe/rendu (`docs/roadmaps/11_boucle.md`). Roadmap 13 (éditeur MVP) **terminée**.
