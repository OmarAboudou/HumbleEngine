...
- Current phase : **le layout** (`docs/roadmaps/14_layout.md`) — donner un vrai moteur de layout à l'éditeur (resize → les panneaux refluent, conteneurs dimensionnés par leur contenu). Modèle **« contraintes ↓ / tailles ↑ »** de Flutter, piloté par signals : **une computation de layout par nœud** — `Incoming` (`Property<Constraints>`, posée par le parent) + `CreateEffect(() => Size.Value = ComputeLayout(Incoming.Value))` ; le parent pose `child.Incoming` et lit `child.Size` (acyclique : ↓ Incoming, ↑ Size). **Skip-unchanged et relayout boundaries gratuits** (les cellules ne notifient que sur changement réel). Flex en **enveloppes-descripteurs** `Expanded`/`Flexible` (pas des `Node`) dissoutes en **parent-data** via `virtual Node.CreateParentData()` au goulot d'adoption ; `FlexParentData` = sac de `Property` ; l'inspecteur lit la parent-data en plus (contextuel). Pas de `DesiredSize`/`PreferredSize`/`dry`. **Différés** : `MinSize`/`MaxSize` (bornes propres, client `Button`), `Move`/`Moved` (réordonner sans ré-adoption, client glisser-déposer), contexte hérité `InheritedData` (roadmap 15, anti-prop-drilling, client `EditorState`). Roadmap 13 (éditeur MVP, blocs 1-4) **terminée**. **Différé** : découplage pompe/rendu (`docs/roadmaps/11_boucle.md`).
